...
public class DataSeeder
{
    ...
    public void Seed()
    {
        ...
        foreach (var business in businesses)
        {
            services = fakerService.Generate(5)
                                   .Select(s => s with
                                   {
                                       BusinessId = business.Id, TaxId = taxes[new Faker().Random.Int(0, taxes.Count - 1)].Id
                                   }).ToList();
        ...
        foreach (var service in services)
        {
            reservations = fakerReservation.Generate(5).Select(r
                => r with { ServiceId = service.Id, EmployeeId = employees[new Faker().Random.Int(0, employees.Count - 1)].Id }).ToList();
        ...
        foreach (var business in businesses)
        {
            giftcards = fakerGiftCard.Generate(5).Select(gc
                => gc with { BusinessId = business.Id, IssuedById = users[new Faker().Random.Int(0, users.Count - 1)].Id }).ToList();
        ...
        foreach (var business in businesses)
        {
            products = fakerProduct.Generate(20)
                                   .Select(p => p with
                                   {
                                       BusinessId = business.Id, TaxId = taxes[new Faker().Random.Int(0, taxes.Count - 1)].Id
                                   }).ToList();
        ...
        foreach (var business in businesses)
        {
            discounts = fakerDiscount.Generate(5).Select(d => d with
            {
        ...
        foreach (var order in orders)
        {
            orderItems = fakerOrderItem.Generate(5).Select(oi => oi with
            {
                ...
                ProductId = new Faker().Random.Bool() ? products[new Faker().Random.Int(0, products.Count - 1)].Id : null,
                ProductVariationId =
                new Faker().Random.Bool() ? productVariations[new Faker().Random.Int(0, productVariations.Count - 1)].Id : null,
        ...
        foreach (var order in orders)
        {
            payments = fakerPayment.Generate(5).Select(p => p with
            {
                ...
                PaymentTerminalId =
                new Faker().Random.Bool() ? paymentTerminals[new Faker().Random.Int(0, paymentTerminals.Count - 1)].Id : (Guid?)null
            }).ToList();
        ...
        foreach (var order in orders)
        {
            refunds = fakerRefund.Generate(5).Select(r => r with
            {
                OrderId = order.Id,
                PaymentId = payments[new Faker().Random.Int(0, payments.Count - 1)].Id,
                UserId = users[new Faker().Random.Int(0, users.Count - 1)].Id
            }).ToList();
